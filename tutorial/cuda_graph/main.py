...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.up_proj = nn.Linear(
            config.hidden_size, config.hidden_size * config.up_ratio
        )
        self.rms_norm = nn.RMSNorm(config.hidden_size * config.up_ratio)
        self.down_proj = nn.Linear(
            config.hidden_size * config.up_ratio, config.hidden_size
        )
...
if __name__ == "__main__":
    ...
    assert len(sys.argv) == 2 and sys.argv[1] in [
        ...
        "normal",
        "all",
    ], "usage: python main.py [cuda_graph, normal, all]"
    ...
    if sys.argv[1] == "cuda_graph":
        ...
        output_cuda_graph = use_cuda_graph(
            model, config, inputs_for_record, inputs_for_run
        )
        output_normal = use_normal(model, config, inputs_for_record, inputs_for_run)
        assert torch.allclose(
            output_cuda_graph, output_normal
        ), "output of cuda graph and normal are not the same"
